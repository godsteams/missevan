<template>
  <div class="albums">
    ...
    <div class="tilte">
      <span></span>全部音单
      <button @click="type">类型</button>
    </div>
    <div class="hide" ref="hide">
      <div class="type_choose">
        <span @click="cancel">取消</span>类型选择
      </div>
      <div class="tags">
...
export default {
  ...
  methods: {
    ...
    classify(i) {
      this.$refs.hide.style.right = "-100%";
      axios.get("/explore/tagalbum?order=0&tid=" + i).then(res => {
        this.music = res.data.albums;
      });
    }
  }
...
.pic {
  width: 164px;
  height: 164px;
  background: url("../assets/img/yuan.png") center center;
...
.font1 {
  width: 36px;
  height: 36px;
  background: url("../assets/img/xb.png") 0 -108px;
  margin: 14px auto;
  background-size: 261px 235px;
}
.left:nth-of-type(2) .font1 {
  background: url("../assets/img/xb.png") -38px -108px;
  margin: 14px auto;
  background-size: 261px 235px;
}
.left:nth-of-type(3) .font1 {
  background: url("../assets/img/xb.png") -76px -108px;
